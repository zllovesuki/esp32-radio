...
int32_t radio_board_init(void)
{
    ...
    if (!(xEventGroupWaitBits(wifi_events, BIT0, false, false, pdMS_TO_TICKS(60000)) & BIT0)) return -1;
    radio_led(0, 12, 4);
    printf("PROBE {\"event\":\"hardware_ready\",\"firmware\":\"rust\",\"wifi\":true,\"psram_bytes\":%u,\"internal_free\":%u}\n",
           (unsigned)esp_psram_get_size(), (unsigned)radio_heap_free());
    return 0;
}

int32_t radio_network_ready(void) { return wifi_events && (xEventGroupGetBits(wifi_events) & BIT0); }
...
int32_t radio_clock_sync(void)
{
    ...
    if (time(NULL) < 1700000000) esp_netif_sntp_sync_wait(pdMS_TO_TICKS(20000));
    return time(NULL) >= 1700000000;
}

uint64_t radio_now_us(void) { return esp_timer_get_time(); }
uint32_t radio_random(void) { return esp_random(); }
uint32_t radio_heap_free(void) { return heap_caps_get_free_size(MALLOC_CAP_INTERNAL); }
uint32_t radio_stack_free(void) { return uxTaskGetStackHighWaterMark(NULL); }
int32_t radio_rssi(void) { wifi_ap_record_t ap = {0}; esp_wifi_sta_get_ap_info(&ap); return ap.rssi; }
...
int32_t radio_console_byte(void) { int ch = getchar(); if (ch == EOF) clearerr(stdin); return ch; }
void radio_log(const char *message) { ESP_LOGI("signaling", "%s", message); }
uint32_t radio_recovery_attempt(int32_t reset)
{
    portENTER_CRITICAL(&recovery_lock);
    uint32_t previous = recovery_attempt;
    if (reset) recovery_attempt = 0;
    else if (recovery_attempt < 3) recovery_attempt++;
    portEXIT_CRITICAL(&recovery_lock);
    return previous;
}
void radio_restart(void) { esp_restart(); abort(); }
...
uint8_t *radio_music_load(size_t *length)
{
    *length = 0;
    ...
    if (!part || part->size != 4 * 1024 * 1024) return NULL;
    uint8_t *bytes = heap_caps_malloc(part->size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (!bytes) return NULL;
    if (esp_partition_read(part, 0, bytes, part->size)) { free(bytes); return NULL; }
    *length = part->size;
    return bytes;
}
void radio_music_free(uint8_t *bytes) { free(bytes); }
